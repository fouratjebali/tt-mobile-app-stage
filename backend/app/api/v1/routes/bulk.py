...
async def generate_bulk(
    ...
    raw_result = await bridge.generate_bulk(
        recipients=request.recipients_payload(),
        topic=request.topic,
        instructions=request.instructions,
    )
    return _to_bulk_response(raw_result)
...
async def send_bulk(
    ...
    raw_result = await bridge.send_bulk(
        recipients=request.recipients_payload(),
        topic=request.topic,
        instructions=request.instructions,
    )
...
def _to_bulk_response(raw_result: str) -> BulkResponse:
