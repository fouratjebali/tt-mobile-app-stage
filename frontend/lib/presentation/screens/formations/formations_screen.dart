...
class _TrainingCalendarSectionState extends State<_TrainingCalendarSection> {
  ...
  Widget build(BuildContext context) {
    ...
    final pageSessions =
        monthSessions
            ...
            .toList();
    final canGoPrevious = !_sameMonth(selectedDate, DateTime(_startYear, 1));
    final canGoNext = !_sameMonth(selectedDate, DateTime(_endYear, 12));
    ...
    return Column(
      ...
      children: [
        ...
        if (sessions.isEmpty)
          ...
          _CalendarMonthToolbar(
            label: _formatMonthLabel(context, selectedDate),
            onPickMonth:
            ...
            onPreviousMonth:
                canGoPrevious ? () => _moveSelectedMonth(-1) : null,
            onNextMonth: canGoNext ? () => _moveSelectedMonth(1) : null,
            tone: widget.tone,
          ),
          const SizedBox(height: 10),
          _CalendarMonthSummary(
            ...
            yearCount: _sessionsForYear(sessions, selectedYear).length,
            tone: widget.tone,
          ),
          const SizedBox(height: 12),
  ...
  Future<void> _openMonthYearPicker(
    ...
    final picked = await showModalBottomSheet<DateTime>(
      ...
      backgroundColor: Colors.transparent,
      builder:
          (context) => _CalendarMonthPickerSheet(
  ...
  void _moveSelectedMonth(int delta) {
    final sortedSessions = _sortSessionsByDate(widget.sessions);
    final selectedYear = _selectedYear ?? _defaultCalendarYear(sortedSessions);
    final selectedMonth =
        _selectedMonth ??
        _defaultCalendarMonthNumber(sortedSessions, selectedYear);
    final next = DateTime(selectedYear, selectedMonth + delta);
    if (next.year < _startYear ||
        next.year > _endYear ||
        (next.year == _startYear && next.month < 1) ||
        (next.year == _endYear && next.month > 12)) {
      return;
    }
    setState(() {
...
class _CalendarMonthToolbar extends StatelessWidget {
  const _CalendarMonthToolbar({
    required this.label,
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      children: [
        Expanded(child: _CalendarDateButton(label: label, onTap: onPickMonth)),
        const SizedBox(width: 8),
        _CalendarIconAction(
          tooltip: l10n.t('formations.previousMonth'),
        ...
        const SizedBox(width: 8),
        _CalendarIconAction(
          tooltip: l10n.t('formations.nextMonth'),
...
class _CalendarDateButton extends StatelessWidget {
  const _CalendarDateButton({required this.label, required this.onTap});
...
class _CalendarIconAction extends StatelessWidget {
  const _CalendarIconAction({
        ...
        child: InkWell(
          ...
          child: Container(
            ...
            child: Icon(
              ...
              color:
                  onTap == null
                      ? tone.muted.withValues(alpha: 0.45)
                      : tone.text,
              size: 21,
            ),
          ),
...
class _CalendarMonthSummary extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final l10n = context.l10n;
    final label = l10n
        .t('formations.calendarSelectionSummary')
        .replaceAll('{month}', _formatMonthLabel(context, selectedDate))
        .replaceAll('{monthCount}', '$sessionCount')
        .replaceAll('{year}', '${selectedDate.year}')
        .replaceAll('{yearCount}', '$yearCount');
    return Text(
      label,
      style: TextStyle(
        color: tone.muted,
...
class _CalendarMonthPickerSheet extends StatefulWidget {
  const _CalendarMonthPickerSheet({
...
class _DraftsSectionState extends State<_DraftsSection> {
  static const int _draftsPerPage = 8;

  int _page = 0;

  @override
  void didUpdateWidget(covariant _DraftsSection oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (oldWidget.drafts != widget.drafts ||
        oldWidget.statusFilter != widget.statusFilter ||
        oldWidget.emailTypeFilter != widget.emailTypeFilter) {
      _page = 0;
    }
  }
  ...
  Widget build(BuildContext context) {
    final l10n = context.l10n;
    final pageCount = _calendarPageCount(widget.drafts.length, _draftsPerPage);
    final pageIndex = _page.clamp(0, pageCount - 1).toInt();
    final pageDrafts =
        widget.drafts
            .skip(pageIndex * _draftsPerPage)
            .take(_draftsPerPage)
            .toList();
    ...
    return Column(
      ...
      children: [
        ...
        _DraftFilterBar(
          ...
          emailTypeFilter: widget.emailTypeFilter,
          tone: widget.tone,
          onStatusFilterChanged: widget.onStatusFilterChanged,
          onEmailTypeFilterChanged: widget.onEmailTypeFilterChanged,
        ),
        const SizedBox(height: 12),
        if (widget.drafts.isEmpty)
          ...
          const SizedBox(height: 12),
          for (final draft in pageDrafts) ...[
            _DraftCard(
              draft: draft,
              tone: widget.tone,
              onTap: () => widget.onOpen(draft),
            ),
            const SizedBox(height: 10),
          ],
          if (pageCount > 1) ...[
...
class _DraftReviewSheetState extends State<_DraftReviewSheet> {
  ...
  Future<void> _markSent() async {
    final saved = _draft.canReview ? await _save() : _draft;
    if (saved == null) return;
    if (!mounted) return;
    setState(() => _saving = true);
    try {
      await widget.viewModel.markDraftManuallySent(saved);
      if (mounted) Navigator.of(context).pop();
    } catch (error) {
      if (mounted) _showSheetMessage(error.toString());
    } finally {
  ...
  void _applySavedDraft(TrainingDraft draft) {
    setState(() {
      _draft = draft;
  ...
  Widget build(BuildContext context) {
    ...
    final canAct = !_draft.isSent && !_draft.isRejected;
              ...
              children: [
                Expanded(
                  child: OutlinedButton.icon(
                    onPressed: _saving || !canAct ? null : _reject,
                    icon: const Icon(Icons.close_rounded),
                ...
                Expanded(
                  child: FilledButton.icon(
                    onPressed: _saving || !canAct ? null : _markSent,
                    icon:
                        _saving
                            ? const SizedBox(
                              width: 18,
                    ...
                    label: Text(l10n.t('formations.markSent')),
                    style: FilledButton.styleFrom(
                      backgroundColor: AppPalette.deepTeal,
                      foregroundColor: AppPalette.white,
                      padding: const EdgeInsets.symmetric(vertical: 14),
